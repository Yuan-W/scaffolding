...
from hints_db_handler import HintsDBHandler
...
hints_db_url = app.config['ADDRESS_HINTS_DB']

hints_handler = HintsDBHandler(hints_db_url, 'hints')
hints_handler.init_db()
...
parser.add_argument('exercise_id', type=int, location='json', required=True, help='exercise_id must exists and be an integer')
...
def store_hints():
    input_hint = request.json
    if ('instructor_id' not in input_hint) or (not isinstance(input_hint['instructor_id'], int)):
        abort(400, {'errors': dict(message="instructor_id must exists and be an integer")})
    instructor_id = input_hint['instructor_id']

    if ('exercise_id' not in input_hint) or (not isinstance(input_hint['exercise_id'], int)):
        abort(400, {'errors': dict(message="exercise_id must exists and be an integer")})
    exercise_id = input_hint['exercise_id']

    if 'hints' not in input_hint:
        abort(400, {'errors': dict(message="hints must exists")})
    hints = input_hint['hints']
    response = hints_handler.postHints(instructor_id, exercise_id, hints)
    if response[1] != 201:
        return response
...
class HintsProvider(Resource):
    ...
    def post(self, **kwargs):
        ...
        response = hints_handler.getHints(exercise_id)
        if response[1] == 200:
